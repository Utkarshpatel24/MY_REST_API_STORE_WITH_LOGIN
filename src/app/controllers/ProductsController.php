...
class ProductsController extends Controller
{
    ...
    public function insertAction()
    {
        ...
        if (count($postdata) > 0) {
            ...
            $id = json_decode(json_encode($result->getinsertedId()), 1)['$oid'];
            $product = $product->searchById($id);
            $this->eventsManager->fire('webhookEvent:sendAddResponse', $this, $product);
    ...
    public function dashboardAction()
    {
        ...
        if (count($postdata) > 0) {
            ...
            foreach ($searchResult as $val) {
                ...
                <div class="d-flex justify-content-around">
                    <a href="#" onclick="abc(`' . $val['_id']['$oid'] . '`)" data-bs-toggle="modal" data-bs-target="#staticBackdrop" ><ion-icon size="large" name="eye-outline"></ion-icon></a>
                    <a href="/products/updateProduct/' . $val['_id']['$oid'] . '"><i class="fa fa-edit" style="font-size:38px;color:light-blue"></i></a>
                    <a href="/products/deleteProduct/' . $val['_id']['$oid'] . '"><ion-icon size="large"  name="trash-outline"></ion-icon></a>
                </div>
    ...
    public function updateProductAction($id)
    {
        ...
        if (count($postdata) > 0) {
            // print_r($postdata);
            $product = new Products;
            $newproduct=$product->updateProduct($id, $postdata['stock']);
            $this->eventsManager->fire('webhookEvent:sendUpdateResponse', $this, $newproduct);
            $this->response->redirect('/products/dashboard');
        }
    }
}
